...
3. Rename the package `src/kuzu_store` to something more meaningful and adjust the Python
   import statements.
4. Replace any occurrence of `kuzu_store` and `kuzu-store` across the repo, according
...
cmd [3] | mypy --install-types --non-interactive src/kuzu_store tests
...
~$ hatch build
[sdist]
dist/kuzu_store-0.0.1.tar.gz

[wheel]
dist/kuzu_store-0.0.1-py3-none-any.whl
```
...
Then set the desired version number in `src/kuzu_store/__about__.py` and tag the commit using the format
`vX.Y.Z`. After pushing the tag, a Github workflow will start and take care of building and releasing the package.

## License

`kuzu-store` is distributed under the terms of the [Apache-2.0](https://spdx.org/licenses/Apache-2.0.html) license.
